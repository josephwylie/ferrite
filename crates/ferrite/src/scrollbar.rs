// Adapted from gpui-component 0.5.1 src/scroll/scrollbar.rs (Apache-2.0).
// Copyright Longbridge. Keep the thumb interaction bounds inside the gutter.
// Upstream keeps prepaint geometry private, so this local copy preserves its
// drag/fade behavior while fixing thumb_hit_bounds. Remove it when an upstream
// upgrade makes hover, click, and cursor bounds agree. Public handle/theme types
// still come from the toolkit; unused upstream entry points ease comparison.
#![allow(dead_code)]
...
impl Element for Scrollbar {
    ...
    fn prepaint(
        ...
        for axis in self.axis.all().into_iter() {
            ...
            let thumb_bounds =
                thumb_hit_bounds(bounds, is_vertical, inset, thumb_start, thumb_length);
...
fn thumb_hit_bounds(
    ...
    // Inset only along the scrolling axis. Insetting a WIDTH-wide hit area
...
mod tests {
    use super::*;
    use gpui::{
        div, prelude::*, Context, Modifiers, MouseButton, Render, ScrollHandle, TestAppContext,
    };

    struct ScrollFixture(ScrollHandle);
    ...
    impl Render for ScrollFixture {
        fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl IntoElement {
            div()
                ...
                .child(crate::components::scrollbar("bar", &self.0))
        }
    }

    #[gpui::test]
    fn thumb_padding_drags_without_jumping(cx: &mut TestAppContext) {
        cx.update(crate::theme::init_components);
        let scroll = ScrollHandle::new();
        let (view, cx) = cx.add_window_view(|_, _| ScrollFixture(scroll.clone()));
        ...
        for x in [185., 199.] {
            ...
            cx.run_until_parked();
            let from = point(px(x), px(20.));
            cx.simulate_mouse_down(from, MouseButton::Left, Modifiers::none());
            assert_eq!(
                scroll.offset().y,
                px(0.),
                "thumb padding must not jump the track"
            );
            // The toolkit throttles dragging with a wall-clock 120 Hz limit.
            std::thread::sleep(Duration::from_millis(12));
            cx.simulate_mouse_move(point(px(x), px(80.)), MouseButton::Left, Modifiers::none());
            cx.run_until_parked();
            assert!(
                scroll.offset().y < px(0.),
                "thumb padding must start a drag"
            );
            cx.simulate_mouse_up(point(px(x), px(80.)), MouseButton::Left, Modifiers::none());
        }
    }
    ...
    fn thumb_hover_bounds_stay_inside_click_and_cursor_gutter() {
        ...
        let thumb_bounds = thumb_hit_bounds(bounds, true, inset, thumb_start, thumb_length);
        let edge = point(thumb_bounds.left() + px(1.), px(20.));
        assert!(thumb_bounds.contains(&edge));
        assert!(
