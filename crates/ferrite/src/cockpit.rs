...
impl CockpitView {
    ...
    fn pump(&mut self, cx: &mut Context<Self>) {
        ...
        for update in &frame {
            ...
            if let Some(index) = self.pane_for(update.thread) {
                self.facts.selected(&self.cockpit, update.thread, &self.panes[index].selected);
            }
        }
...
impl CockpitView {
    ...
    fn toggle_tool(
        ...
        self.panes[index].rich.clear_output_selection(&self.panes[index].text_namespace(), cx);
...
mod tests {
    ...
    fn clicking_the_tool_chevron_replaces_the_compact_line_with_selectable_output(
        ...
        let (output_state, expected) = view.read_with(cx, |view, cx| {
            let block = view.cockpit.thread(thread).unwrap().transcript().blocks()
                .iter().find(|block| matches!(&block.body, Body::Tool(tool) if tool.call == "toolu_9")).unwrap();
            let Body::Tool(tool) = &block.body else { unreachable!() };
            let id = format!("output-{}-{:?}-result", view.panes[0].text_namespace(), block.id);
            (crate::rich::testing::output(&id, cx).unwrap(), tool.output.as_ref().unwrap().text.clone())
        });
        let bounds = output_state.read_with(cx, |state, _| state.text_bounds().unwrap());
        assert!(bounds.size.height < px(350.), "large output has a bounded native viewport");
        cx.simulate_click(bounds.center(), gpui::Modifiers::none());
        tick(cx);
        cx.update(|window, cx| assert!(output_state.focus_handle(cx).is_focused(window), "progress pumps preserve native output focus"));
        cx.simulate_keystrokes("cmd-down");
        cx.run_until_parked();
        output_state.read_with(cx, |state, _| {
            assert_eq!(state.cursor(), expected.len(), "native navigation reaches the retained tail");
            assert!(state.scroll_offset().y < px(0.), "the native viewer scrolls to its tail");
        });
    ...
    #[gpui::test]
    fn streaming_output_preserves_native_selection_scroll_and_command_identity(cx: &mut TestAppContext) {
        ...
        fake.streams.borrow()[0].send(SessionEvent::ToolStarted {
            id: "stream".into(), name: "Bash".into(),
            input: serde_json::json!({"command": command}),
        }).unwrap();
        fake.streams.borrow()[0].send(SessionEvent::ToolOutputDelta {
            id: "stream".into(), text: original.clone(),
        }).unwrap();
        tick(cx);
        let chevron = view.read_with(cx, |view, _| view.panes[0].tool_bounds("stream").unwrap().center());
        cx.simulate_click(chevron, gpui::Modifiers::none());
        cx.run_until_parked();
        let (command_state, result_state) = view.read_with(cx, |view, cx| {
            let block = view.cockpit.thread(thread).unwrap().transcript().blocks().iter()
                .find(|block| matches!(&block.body, Body::Tool(tool) if tool.call == "stream")).unwrap();
        ...
        cx.run_until_parked();
        let (selected, cursor, scroll) = result_state.read_with(cx, |state, _| (state.selected_value(), state.cursor(), state.scroll_offset()));
        assert!(!selected.is_empty());
        assert!(scroll.y < px(0.));
        fake.streams.borrow()[0].send(SessionEvent::ToolOutputDelta {
            id: "stream".into(), text: "new output\n".into(),
        }).unwrap();
        tick(cx);
        ...
        fake.streams.borrow()[0].send(SessionEvent::ToolCompleted {
            id: "stream".into(), output: "λ🙂\n".repeat(1500), is_error: false,
            result: ferrite_core::ToolResult::Opaque,
        }).unwrap();
        tick(cx);
        result_state.read_with(cx, |state, _| {
