...
fn native_patch_update_replaces_hunks_without_finishing_the_tool() {
    ...
    assert_eq!(t.diffs[0].path, "b.txt");
    assert_eq!(t.state, ferrite_core::transcript::ToolState::Running);
}
...
fn native_turn_diff_is_a_replacement_snapshot_not_an_invented_tool() {
    ...
    let a = fold(r.drain());
    let t = a.view().main().transcript();
    let d = t
        .turn_diff()
        .expect("shared UI must have native aggregate diff");
    assert_eq!(d.turn_id, "turn");
    assert_eq!(d.diff, "--- a/file\n+++ b/file\n@@ -1 +1 @@\n-old\n+new\n");
    assert!(!t
        .blocks()
        .iter()
...
fn native_diff_updates_survive_store_replay() {
    ...
    assert_eq!(
        a.view().main().transcript().turn_diff().unwrap().diff,
        "persisted aggregate"
    );
...
fn child_patch_updates_target_the_childs_scoped_tool() {
    ...
    let a = fold(r.drain());
    let child = a.view().children().into_iter().next().unwrap();
    ...
    assert_eq!(
        child.transcript().turn_diff().unwrap().diff,
        "child aggregate"
    );
